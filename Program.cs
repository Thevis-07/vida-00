...
public enum TipoEvento { Nascimento, Escola, Conquista, Trabalho, Projeto, Religiao, Relacao, Saude, MudancaCidade, Outro }
...
public class Pessoa
{
    ...
    public Pessoa(string nome, DateTime nascimento, string projetoInicial)
    {
        ...
        Papeis.Add("Filho(a)");
        LinhaDoTempo.Add(new EventoDeVida(nascimento, TipoEvento.Nascimento, "Nasceu"));
    }
    ...
    public void AvancarFase(FaseDeVida nova, string motivo)
    {
        ...
        LinhaDoTempo.Add(new EventoDeVida(DateTime.Now, TipoEvento.Outro, $"Evoluiu para {Fase} ({motivo})"));
    ...
    public void ApresentarResumo()
    {
        ...
        foreach (var e in LinhaDoTempo.OrderBy(x => x.Data))
            Console.WriteLine($"- {e.Data:yyyy-MM-dd} • {e.Tipo}: {e.Descricao}");
        Console.WriteLine("=========================\n");
    }
...
public static class DemoVida
{
    public static void Rodar()
    {
        ...
        // Reorientação de projeto
        pessoa.DefinirProjeto("Solucionador de problemas");
